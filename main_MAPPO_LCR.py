...
def save_params_to_json(params, filename_prefix="params",output_path='data'):
    ...
    src_file='main_MAPPO_LCR.py'
    dst_file=f'{output_path}/{src_file}'
    shutil.copy2(src_file, dst_file)
    src_file='MAPPO_LCR.py'
    dst_file=f'{output_path}/{src_file}'
    shutil.copy2(src_file, dst_file)
...
async def main(args):
    ...
    # 实验参数设置
    fontsize=16
    r_values = [4.1,4.7]#[4.5,4.6,4.7,4.8,4.9,5.0,5.1]#[3.6, 3.8, 4.7, 5.0, 5.5, 6.0] #[3.0, 5.0, 7.0, 9.0]  # 公共物品乘数
    ...
    output_path=f'data/MAPPO_LCR_{formatted_time}_q{str(args.question)}_e_{args.epochs}_L_{args.L_num}_a_{args.alpha}_g_{args.gamma}_ce_{args.clip_epsilon}_gl_{args.gae_lambda}_p_{args.ppo_epochs}_b_{args.batch_size}_delta_{args.delta}_rho_{args.rho}_zeta_{args.zeta}_seed_{args.seed}'
...
if __name__ == "__main__":
    ...
    parser.add_argument('-zeta', type=float, default=3, help='lcr zeta')
